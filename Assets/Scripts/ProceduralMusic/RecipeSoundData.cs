...
{
    [CreateAssetMenu(fileName = "RecipeSoundData", menuName = "Scriptable Objects/ProceduralMusic/RecipeSoundData")]
    public class RecipeSoundData : ScriptableObject
    {
        [Header("Producer")]
        [Tooltip("Machine (MachineData SO) that produces recipes this sound data applies to (optional).")]
        ...
        [Tooltip("Optional override to choose a specific MachineSoundData profile for this recipe (if left empty, machine's profile is used).")]
        public MachineSoundData overrideMachineSoundProfile;

        #region Percussion pattern controls (kept simple)
        ...
        // Returns true when the given beat/subdivision position matches this recipe's pattern.
